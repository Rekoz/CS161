...
;
; Helper function to generate a clause
(defun at-most-one-color-helper (n c m k)
  (cond
      ((> m k) NIL)
      (T (cons (list (- (node2var n c k)) (- (node2var n m k))) (at-most-one-color-helper n c (+ m 1) k)))
    )
  )
...
(defun generate-edge-clauses-helper (e c k)
  (cond
    ((> c k) NIL)
    (T (cons (list (- (node2var (first e) c k)) (- (node2var (second e) c k))) (generate-edge-clauses-helper e (+ c 1) k)))
    )
  )
